...
resource aiMultiServicePrivateEndpoint 'Microsoft.Network/privateEndpoints@2024-05-01' = {
  name: '${aiMultiServiceAccountName}-pe'
  location: 'eastus2'
  properties: {
    subnet: {
      id: resourceId('Microsoft.Network/virtualNetworks/subnets', virtualNetworkName, 'default')
    ...
    privateLinkServiceConnections: [
      {
        name: 'aiMultiServiceConnection'
        properties: {
          privateLinkServiceId: aiMultiService.id
          groupIds: [ 'account' ]
        }
      }
    ]
...
resource sharedPrivateLinkSearchToAI 'Microsoft.Search/searchServices/sharedPrivateLinkResources@2025-02-01-preview' = {
  name: 'search-to-ai-link'
  parent: searchService
  dependsOn: [searchPrivateEndpoint]
  properties: {
    privateLinkResourceId: aiMultiServicePrivateEndpoint.id
    groupId: 'account'
    requestMessage: 'Linking AI Multi-service account to Search Service'
  }
}

resource sharedPrivateLinkSearchToStorage 'Microsoft.Search/searchServices/sharedPrivateLinkResources@2025-02-01-preview' = {
  name: 'search-to-storage-link'
  parent: searchService
  dependsOn: [searchPrivateEndpoint]
  properties: {
    privateLinkResourceId: storageBlobPrivateEndpoint.id
    groupId: 'blob'
    requestMessage: 'Linking Storage Account to Search Service'
  }
}
